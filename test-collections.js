...
async function testAllDates() {
  if (typeof useGameStore === "undefined") {
    console.error(
      "Error: useGameStore is not available. Make sure you are running this script in the browser console while the app is running.",
    );
    return;
  }
  ...
  for (const test of testDates) {
    ...
    await useGameStore
      .getState()
      .testWordCollectionsForDate(test.month, test.day);

    // Log collections that are date-specific (have start/end dates)
    const seasonal = useGameStore
      .getState()
      .wordCollections.filter((c) => c.startDate || c.endDate);

    if (seasonal.length > 0) {
      console.log(
        `Found ${seasonal.length} seasonal collections for this date:`,
      );
...
console.log(
  "Copy this entire script and paste it into your browser console while running the app.",
);
