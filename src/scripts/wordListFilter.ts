 ...
 */
async function filterWordList(words: string[], graphDataPath?: string): Promise<{
  totalWords: number;
  validWords: string[];
  invalidWords: string[];
  percentValid: number;
}> {
  // Default path to the graph data
  const defaultPath = path.join(__dirname, '..', 'data', 'graph.json');
  ...
  try {
    ...
    console.log(`Valid words: ${validWords.length} (${percentValid.toFixed(1)}%)`);
...
function formatOutput(
  filteredResult: { totalWords: number; validWords: string[]; invalidWords: string[]; percentValid: number },
  format: 'js' | 'json' = 'js',
  outputPath?: string
): string | void {
  if (format === 'js') {
