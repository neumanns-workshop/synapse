...
describe("filterCollectionsByDate", () => {
  ...
  describe("Misconfigured Collections", () => {
    it("should treat collection with only startDate as active and warn", () => {
      ...
      const filtered = filterCollectionsByDate(mockCollections, testDate);
      expect(filtered.some((c) => c.id === "misconfigured_start_only")).toBe(
        true,
      );
      expect(consoleWarnSpy).toHaveBeenCalledWith(
        expect.stringContaining(
          'Collection "misconfigured_start_only" is misconfigured',
        ),
      );
    });
    ...
    it("should treat collection with only endDate as active and warn", () => {
      const testDate = new Date(2023, 5, 1); // Any date
      const filtered = filterCollectionsByDate(mockCollections, testDate);
      expect(filtered.some((c) => c.id === "misconfigured_end_only")).toBe(
        true,
      );
      expect(consoleWarnSpy).toHaveBeenCalledWith(
        expect.stringContaining(
          'Collection "misconfigured_end_only" is misconfigured',
        ),
      );
    });
  });
...
describe("testCollectionsForDate", () => {
  ...
  const halloweenCollectionFromDefs = originalAllWordCollections.find(
    (c) => c.id === "halloween",
  );
  const valentinesCollectionFromDefs = originalAllWordCollections.find(
    (c) => c.id === "valentines",
  );
  ...
  it("should return Halloween collection if date is Oct 31", () => {
    if (!halloweenCollectionFromDefs) {
      ...
      console.log(
        "Skipping Halloween test as it's not in current definitions.",
      );
      return;
    }
  ...
  it("should NOT return Halloween collection if date is Oct 1", () => {
    if (
      !halloweenCollectionFromDefs ||
      !halloweenCollectionFromDefs.startDate
    ) {
      console.log(
        "Skipping Halloween test as it's not in current definitions or misconfigured for this test.",
      );
      return;
    }
    // Assuming Halloween starts Oct 15 as per typical definition
    const halloweenStartDateDay =
      halloweenCollectionFromDefs.startDate.getDate();
    if (halloweenStartDateDay > 1) {
      // Only run if Halloween doesn't start on the 1st
      const filtered = testCollectionsForDate(
        originalAllWordCollections,
        10,
        1,
      );
      ...
      console.log(
        "Skipping Halloween Oct 1 test as definition starts on or before the 1st.",
      );
    }
  });
  ...
  it("should return Valentines collection if date is Feb 14", () => {
    if (!valentinesCollectionFromDefs) {
      console.log(
        "Skipping Valentines test as it's not in current definitions.",
      );
      return;
    }
  ...
  it("should return always active collections regardless of date for testCollectionsForDate", () => {
    const alwaysActive = originalAllWordCollections.filter(
      (c) => !c.startDate && !c.endDate,
    );
    if (alwaysActive.length === 0) {
      console.log(
        "Skipping always active test for testCollectionsForDate as none are defined.",
      );
      return;
    }
    ...
    const filteredAlwaysActiveIds = filtered
