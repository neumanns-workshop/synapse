...
describe("Seasonal Word Collections Logic", () => {
  ...
  describe("isCollectionAvailable", () => {
    describe("Always Active Collections", () => {
      it("should return true for collections with no start/end dates", () => {
        ...
        expect(isCollectionAvailable(alwaysActive, new Date(2023, 5, 15))).toBe(
          true,
        );
      });
    });
  ...
  describe("getAllWordCollectionsWithStatus", () => {
    it("should return collections with availability status", async () => {
      ...
      const result = await getAllWordCollectionsWithStatus(
        testCollections,
        mockGraphData,
        testDate,
      );
  ...
  describe("getFilteredWordCollections", () => {
    it("should return filtered collections based on date and graph data", async () => {
      ...
      const result = await getFilteredWordCollections(
        testCollections,
        mockGraphData,
        testDate,
      );
  ...
  describe("Edge Cases and Error Handling", () => {
    ...
    it("should handle leap year dates correctly", () => {
      ...
      expect(isCollectionAvailable(leapYearCollection, leapYearDate)).toBe(
        true,
      );
      ...
      expect(isCollectionAvailable(leapYearCollection, nonLeapYearDate)).toBe(
        true,
      );
    });
