...
describe('Epicycles Upon Epicycles Achievement', () => {
  ...
  it('should return true if at least two different words are revisited (game won)', () => {
    ...
    mockGameStatus = 'won';
    expect(epicyclesUponEpicyclesAchievement.check(mockGameReport as GameReport, mockGameStatus)).toBe(true);
  });
  ...
  it('should return false if only one distinct word is revisited multiple times', () => {
    ...
    expect(epicyclesUponEpicyclesAchievement.check(mockGameReport as GameReport, mockGameStatus)).toBe(false);
  });
