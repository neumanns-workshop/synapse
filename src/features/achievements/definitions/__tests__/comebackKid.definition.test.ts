...
describe('Comeback Kid Achievement', () => {
  ...
  it('should return true if game is won and player was further from target than at start', () => {
    mockGameReport.optimalChoices = [
      { playerPosition: 's', playerChose: 'a', optimalChoice: 'b', isGlobalOptimal: false, isLocalOptimal: true, hopsFromPlayerPositionToEnd: 2 },
      { playerPosition: 'a', playerChose: 'x', optimalChoice: 'y', isGlobalOptimal: false, isLocalOptimal: false, hopsFromPlayerPositionToEnd: 4 }, // Further than initial 3 hops
      { playerPosition: 'x', playerChose: 'e', optimalChoice: 'e', isGlobalOptimal: true, isLocalOptimal: true, hopsFromPlayerPositionToEnd: 1 },
    ];
    expect(comebackKidAchievement.check(mockGameReport as GameReport, mockGameStatus)).toBe(true);
  });

  it('should return false if game is won but player was never further from target than at start', () => {
    mockGameReport.optimalChoices = [
      { playerPosition: 's', playerChose: 'a', optimalChoice: 'b', isGlobalOptimal: false, isLocalOptimal: true, hopsFromPlayerPositionToEnd: 2 },
      { playerPosition: 'a', playerChose: 'c', optimalChoice: 'c', isGlobalOptimal: true, isLocalOptimal: true, hopsFromPlayerPositionToEnd: 1 },
    ];
    expect(comebackKidAchievement.check(mockGameReport as GameReport, mockGameStatus)).toBe(false);
  });

  it('should return false if hopsFromPlayerPositionToEnd is never defined', () => {
    mockGameReport.optimalChoices = [
      { playerPosition: 's', playerChose: 'a', optimalChoice: 'b', isGlobalOptimal: false, isLocalOptimal: true }, // hops undefined
    ];
    expect(comebackKidAchievement.check(mockGameReport as GameReport, mockGameStatus)).toBe(false);
  });
