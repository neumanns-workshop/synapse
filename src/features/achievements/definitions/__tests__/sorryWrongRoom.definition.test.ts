...
describe("Sorry, Wrong Room Achievement", () => {
  ...
  it("should return true if game won and backtrackEvents length > 3", () => {
    ...
    mockGameStatus = "won";
    expect(
      sorryWrongRoomAchievement.check(
  ...
  it("should return true if game given_up and backtrackEvents length > 3", () => {
    mockGameReport.backtrackEvents = Array(5)
      .fill(null)
      .map((_, i) => createBacktrackEvent(i)); // Length 5
  ...
  it("should return false if backtrackEvents length is 3 or less", () => {
    mockGameReport.backtrackEvents = Array(3)
      .fill(null)
      .map((_, i) => createBacktrackEvent(i)); // Length 3
    expect(
      sorryWrongRoomAchievement.check(
        mockGameReport as GameReport,
        mockGameStatus,
  ...
  it("should return false if game status is not 'won' or 'given_up'", () => {
    ...
    mockGameReport.backtrackEvents = Array(4)
      .fill(null)
      .map((_, i) => createBacktrackEvent(i));
    expect(
      sorryWrongRoomAchievement.check(
        mockGameReport as GameReport,
        mockGameStatus,
