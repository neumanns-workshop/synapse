...
describe('Déjà Vu Achievement', () => {
  ...
  it('should return true if a word is revisited in playerPath (game won)', () => {
    ...
    mockGameStatus = 'won';
    expect(dejaVuAchievement.check(mockGameReport as GameReport, mockGameStatus)).toBe(true);
  });
  ...
  it('should return false if no word is revisited', () => {
    ...
    expect(dejaVuAchievement.check(mockGameReport as GameReport, mockGameStatus)).toBe(false);
  });
