...
describe('Dancing to a Different Beat Achievement', () => {
  ...
  it('should return true if won and no move except the last was globally optimal', () => {
    ...
    expect(dancingToADifferentBeatAchievement.check(mockGameReport as GameReport, mockGameStatus)).toBe(true);
  });
  ...
  it('should return false if an early move was globally optimal', () => {
    ...
    expect(dancingToADifferentBeatAchievement.check(mockGameReport as GameReport, mockGameStatus)).toBe(false);
  });
  ...
  it('should return false if game status is not \'won\'', () => {
    ...
    mockGameReport.optimalChoices = [createOptimalChoice(false), createOptimalChoice(true)];
    expect(dancingToADifferentBeatAchievement.check(mockGameReport as GameReport, mockGameStatus)).toBe(false);
  });
