...
import type { GraphData, DefinitionsData, WordFrequencies } from '../../services/dataLoader'; // Adjust path for types
import type { PersistentGameState as SavedGame } from '../../services/StorageService'; // Corrected type for saved game
...
const mockedLoadGraphData = dataLoader.loadGraphData as jest.MockedFunction<typeof dataLoader.loadGraphData>;
const mockedLoadDefinitionsData = dataLoader.loadDefinitionsData as jest.MockedFunction<typeof dataLoader.loadDefinitionsData>;
const mockedLoadWordFrequencies = dataLoader.loadWordFrequencies as jest.MockedFunction<typeof dataLoader.loadWordFrequencies>;

// Mock StorageService functions
import * as storageService from '../../services/StorageService';
jest.mock('../../services/StorageService');
const mockedLoadCurrentGame = storageService.loadCurrentGame as jest.MockedFunction<typeof storageService.loadCurrentGame>;
// We might need to mock other StorageService functions like saveCurrentGame, clearCurrentGame if loadInitialData or subsequent actions call them directly or indirectly in a way that affects the test.
...
const mockedGetFilteredWordCollections = wordCollectionsManager.getFilteredWordCollections as jest.MockedFunction<typeof wordCollectionsManager.getFilteredWordCollections>;
...
describe('useGameStore - loadInitialData', () => {
  ...
  it('should load initial data successfully and set state correctly when no saved game exists', async () => {
    ...
    const mockGraph: GraphData = { wordA: { edges: {wordB: 1}, tsne: [1,1] } };
    ...
    expect(mockedGetFilteredWordCollections).toHaveBeenCalledWith(wordCollectionsManager.allWordCollections, mockGraph);
  });
  ...
  it('should restore a saved game in "playing" state', async () => {
    ...
    expect(state.pathDisplayMode.optimal).toBe(savedGameData.pathDisplayMode.optimal);
    // Check that suggestedPathFromCurrent was recalculated
    expect(state.suggestedPathFromCurrent).toEqual(['wordB', 'wordC']);
    expect(state.potentialRarestMovesThisGame).toEqual(savedGameData.potentialRarestMovesThisGame || []);
  ...
  it('should not restore a saved game if its status is not "playing"', async () => {
    // Arrange
    const mockGraph: GraphData = { wordA: { edges: { wordB: 1 }, tsne: [0,0] } }; // Minimal graph
    const mockDefinitions: DefinitionsData = { wordA: ['def'] };
    const mockWordFrequencies: WordFrequencies = { wordA: 10 };
    const mockCollections: WordCollection[] = [];
...
describe('useGameStore - startGame', () => {
  const getInitialDataFields = () => ({
    graphData: null, definitionsData: null, wordFrequencies: null, isLoadingData: false,
    errorLoadingData: null, startWord: null, targetWord: null, currentWord: null,
    playerPath: [], optimalPath: [], suggestedPathFromCurrent: [], potentialRarestMovesThisGame: [],
    pathDisplayMode: { player: true, optimal: false, suggested: false, ai: false },
    gameStatus: "idle" as const, gameReport: null, optimalChoices: [], backtrackHistory: [],
    aboutModalVisible: false, statsModalVisible: false, definitionDialogWord: null,
    definitionDialogPathIndex: null, definitionDialogVisible: false, selectedAchievement: null,
    achievementDialogVisible: false, wordCollections: [], activeWordCollections: [],
    isChallenge: false, shouldRestoreTempGame: false, hasPendingChallenge: false,
    pendingChallengeWords: null,
  });

  // Mocks for StorageService functions that startGame might call
  // mockedLoadCurrentGame is already defined above for loadInitialData tests
  const mockedSaveCurrentGame = storageService.saveCurrentGame as jest.MockedFunction<typeof storageService.saveCurrentGame>;
  const mockedClearCurrentGame = storageService.clearCurrentGame as jest.MockedFunction<typeof storageService.clearCurrentGame>;
  const mockedRestoreTempGame = storageService.restoreTempGame as jest.MockedFunction<typeof storageService.restoreTempGame>;
  ...
  it('should successfully start a new game', async () => {
    ...
    const mockGraph: GraphData = {
      ...
      oS1: { edges: { wordS: 1}, tsne: [1,1]}, oS2: { edges: {wordS:1}, tsne:[2,2]},
      oM11: { edges: { wordM1: 1}, tsne: [11,11]},
      oE1: { edges: { wordE: 1}, tsne: [101,101]}, oE2: { edges: {wordE:1}, tsne:[102,102]},
  ...
  it('should restore a temporary game if shouldRestoreTempGame is true and temp game exists', async () => {
    // Arrange
    const mockGraph: GraphData = { /* ... minimal graph ... */ 
      wordA: { edges: { wordB: 1 }, tsne: [0,0] },
      wordB: { edges: { wordA: 1, wordC: 1 }, tsne: [1,1] },
      wordC: { edges: { wordB: 1 }, tsne: [2,2] },
    };
    useGameStore.setState({ graphData: mockGraph, shouldRestoreTempGame: true });
    ...
    const tempGameData: SavedGame = {
      ...
      gameStatus: 'playing',
      optimalChoices: [{ playerPosition: 'tempA', playerChose: 'tempB', optimalChoice: 'tempB', isGlobalOptimal: true, isLocalOptimal: true }],
      backtrackHistory: [],
    ...
    expect(state.suggestedPathFromCurrent).toEqual(tempGameData.suggestedPathFromCurrent);
    expect(state.gameStatus).toBe('playing');
    ...
    const findValidWordPairSpy = jest.spyOn(useGameStoreModule, 'findValidWordPair');
