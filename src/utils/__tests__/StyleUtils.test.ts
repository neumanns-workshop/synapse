...
describe('StyleUtils', () => {
  describe('getPointerEventStyles', () => {
    ...
    it('should handle all valid pointer event values', () => {
      ...
      expect(getPointerEventStyles('box-none')).toEqual({ pointerEvents: 'box-none' });
      expect(getPointerEventStyles('box-only')).toEqual({ pointerEvents: 'box-only' });
    });
  });
  ...
  describe('getShadowStyles', () => {
    beforeEach(() => {
      ...
      (Platform.select as jest.Mock).mockImplementation(obj => obj[Platform.OS]);
    });
