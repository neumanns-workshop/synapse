...
import type { OptimalChoice, PotentialRarestMove, BacktrackReportEntry, GameReport } from '../gameReportUtils';
...
const mockGraphData: GraphData = {
  ...
  noEdgeWord: { // Word exists but has no outgoing edges in its 'edges' object
    edges: {},
    tsne: [0.9, 1.0],
  },
  wordWithNoEdgesField: { // Word exists but is missing the 'edges' field entirely (requires type assertion for mock)
    tsne: [0.95, 1.05],
...
describe('gameReportUtils', () => {
  describe('getSemanticDistance', () => {
    it('should return 1 - similarity for direct connections', () => {
      expect(getSemanticDistance(mockGraphData, 'wordA', 'wordB')).toBeCloseTo(0.2);
      expect(getSemanticDistance(mockGraphData, 'wordA', 'wordC')).toBeCloseTo(0.5);
      expect(getSemanticDistance(mockGraphData, 'wordB', 'wordD')).toBeCloseTo(0.1);
    });
    ...
    it('should return 1 if the source word is not in graphData', () => {
      expect(getSemanticDistance(mockGraphData, 'unknownWord', 'wordA')).toBe(1);
    });

    it('should return 1 if the target word is not in the source word\'s edges', () => {
      expect(getSemanticDistance(mockGraphData, 'wordA', 'unknownWord')).toBe(1);
    });

    it('should return 1 if both words are not in graphData', () => {
      expect(getSemanticDistance(mockGraphData, 'unknown1', 'unknown2')).toBe(1);
    });
    ...
    it('should return 1 if source word exists but is missing the edges field entirely', () => {
      expect(getSemanticDistance(mockGraphData, 'wordWithNoEdgesField', 'wordA')).toBe(1);
    });
  });
  ...
  describe('calculateTotalSemanticDistance', () => {
    ...
    it('should correctly calculate distance for a valid path with direct connections', () => {
      ...
      const path = ['wordA', 'wordB', 'wordD'];
      expect(calculateTotalSemanticDistance(path, mockGraphData)).toBeCloseTo(0.3);
    });

    it('should sum distances including 1 for missing direct links', () => {
      // wordA -(no link => dist 1)-> wordD -(sim 0.9 => dist 0.1)-> wordB. Total = 1.1
      const path = ['wordA', 'wordD', 'wordB'];
      expect(calculateTotalSemanticDistance(path, mockGraphData)).toBeCloseTo(1.1);
    });
    ...
    it('should handle path segments where a word has no edges defined', () => {
      ...
      const path = ['wordA', 'wordB', 'noEdgeWord', 'wordA'];
      expect(calculateTotalSemanticDistance(path, mockGraphData)).toBeCloseTo(2.2);
    });
    ...
    it('should produce same results as calculatePathDistance for various paths', () => {
      ...
      pathsToTest.forEach(path => {
        expect(calculateTotalSemanticDistance(path, mockGraphData))
          .toBe(calculatePathDistance(mockGraphData, path));
      });
    });
  });
  ...
  describe('trackOptimalChoice', () => {
    ...
    it('should correctly identify choseMostSimilarNeighbor', () => {
      ...
      const resultMostSimilar = trackOptimalChoice(mockGraphData, playerPosition, playerChoiceMostSimilar, optimalPath, suggestedPath, targetWord, findShortestPath, mockWordFrequencies);
      expect(resultMostSimilar.choseMostSimilarNeighbor).toBe(true);

      const resultNotMostSimilar = trackOptimalChoice(mockGraphData, playerPosition, playerChoiceNotMostSimilar, optimalPath, suggestedPath, targetWord, findShortestPath, mockWordFrequencies);
    ...
    it('should correctly identify choseLeastSimilarNeighbor', () => {
      ...
      const resultLeastSimilar = trackOptimalChoice(mockGraphData, playerPosition, playerChoiceLeastSimilar, optimalPath, suggestedPath, targetWord, findShortestPath, mockWordFrequencies);
      expect(resultLeastSimilar.choseLeastSimilarNeighbor).toBe(true);

      const resultNotLeastSimilar = trackOptimalChoice(mockGraphData, playerPosition, playerChoiceNotLeastSimilar, optimalPath, suggestedPath, targetWord, findShortestPath, mockWordFrequencies);
    ...
    it('should handle a single neighbor correctly for similar/least similar checks', () => {
      ...
      const result = trackOptimalChoice(graphSingleNeighbor, playerPosition, playerChoice, optimalPath, suggestedPath, 'end', findShortestPath, mockWordFrequencies);
    ...
    it('should correctly identify choseRarestNeighbor', () => {
      ...
      const resultRarest = trackOptimalChoice(mockGraphData, playerPosition, playerChoiceRarest, optimalPath, suggestedPath, targetWord, findShortestPath, mockWordFrequencies);
      expect(resultRarest.choseRarestNeighbor).toBe(true);

      const resultNotRarest = trackOptimalChoice(mockGraphData, playerPosition, playerChoiceNotRarest, optimalPath, suggestedPath, targetWord, findShortestPath, mockWordFrequencies);
    ...
    it('should handle no direct neighbors for similarity/rarity checks', () => {
      ...
      const result = trackOptimalChoice(mockGraphData, playerPosition, playerChoice, optimalPath, suggestedPath, targetWord, findShortestPath, mockWordFrequencies);
    ...
    it('should handle missing wordFrequencies data gracefully for rarity check', () => {
      ...
      const result = trackOptimalChoice(mockGraphData, playerPosition, playerChoice, optimalPath, suggestedPath, targetWord, findShortestPath, null /* no wordFrequencies */);
      expect(result.choseRarestNeighbor).toBe(false); // Should default to false if no frequency data
    });

     it('should calculate hopsFromPlayerPositionToEnd based on suggestedPath', () => {
      const playerPosition = 'wordA';
      const playerChoice = 'wordB';
      const optimalPath = ['wordA', 'wordB', 'wordC', 'wordD'];
      // wordA is at index 0 of suggestedPath. Path is [A,B,C,D]. Length 4. Hops = 4-1 = 3
      const suggestedPath = ['wordA', 'wordB', 'wordC', 'wordD']; 
      
      const result = trackOptimalChoice(mockGraphData, playerPosition, playerChoice, optimalPath, suggestedPath, targetWord, findShortestPath, mockWordFrequencies);
      expect(result.hopsFromPlayerPositionToEnd).toBe(3);

      const suggestedPathShorter = ['wordA', 'wordX', 'wordD']; // Length 3, Hops = 2
      const resultShorter = trackOptimalChoice(mockGraphData, playerPosition, playerChoice, optimalPath, suggestedPathShorter, targetWord, findShortestPath, mockWordFrequencies);
      expect(resultShorter.hopsFromPlayerPositionToEnd).toBe(2);

      const emptySuggestedPath: string[] = [];
      const resultEmptySuggested = trackOptimalChoice(mockGraphData, playerPosition, playerChoice, optimalPath, emptySuggestedPath, targetWord, findShortestPath, mockWordFrequencies);
      expect(resultEmptySuggested.hopsFromPlayerPositionToEnd).toBe(Infinity);
    });
  ...
  describe('generateGameReport', () => {
    ...
    const mockRecordedOptimalChoices: OptimalChoice[] = [
      {
        playerPosition: 'wordA', playerChose: 'wordB', optimalChoice: 'wordB',
        isGlobalOptimal: true, isLocalOptimal: true,
        hopsFromPlayerPositionToEnd: 2, choseMostSimilarNeighbor: true, choseLeastSimilarNeighbor: false, choseRarestNeighbor: false,
      },
      {
        playerPosition: 'wordB', playerChose: 'wordD', optimalChoice: 'wordD',
        isGlobalOptimal: true, isLocalOptimal: true,
        hopsFromPlayerPositionToEnd: 0, choseMostSimilarNeighbor: false, choseLeastSimilarNeighbor: false, choseRarestNeighbor: true,
      },
    ];
    ...
    it('should generate a correct report for a winning game path', () => {
      ...
      // semanticPathEfficiency: (optimalDistance / playerDistance) * 100 = (0.3 / 0.3) * 100 = 100
      expect(report.semanticPathEfficiency).toBeCloseTo(100);
    ...
    it('should generate a correct report for a game given up before reaching targetWord', () => {
      ...
      const recordedChoicesPartial: OptimalChoice[] = [
        { 
          playerPosition: 'wordA', playerChose: 'wordC', optimalChoice: 'wordB', 
          isGlobalOptimal: false, isLocalOptimal: true, // Assuming wordC was locally suggested
          hopsFromPlayerPositionToEnd: 1, // e.g. if suggested path from A via C was A-C-D
          choseMostSimilarNeighbor: false, choseLeastSimilarNeighbor: true, choseRarestNeighbor: false,
        }
      ];
      ...
      // semanticPathEfficiency:
      // suggestedPathFromFinalPosition (wordC to wordD): C -> A (0.5) -> B (0.2) -> D (0.1) NO, this is not right.
      // findShortestPath(mockGraphData, 'wordC', 'wordD') -> ['wordC', 'wordA', 'wordB', 'wordD'] cost 0.5+0.2+0.1 = 0.8. OR ['wordC', 'wordD'] cost 0.7.
      // findShortestPath returns ['wordC', 'wordD'] as it's shorter (cost 0.7).
      // hypotheticalFullPath: [wordA] + [wordC, wordD] (from C to D) -> [wordA, wordC, wordD]
      // efficiencyDenominator (dist of A-C-D): dist(A-C) + dist(C-D) = 0.5 + 0.7 = 1.2
      // semanticPathEfficiency = (optimalSemanticDistance / efficiencyDenominator) * 100 = (0.3 / 1.2) * 100 = 25
      expect(report.semanticPathEfficiency).toBeCloseTo(25);
      expect(report.missedOptimalMoves.length).toBe(1); // Chose C instead of B at wordA
      expect(report.missedOptimalMoves[0]).toContain('At wordA, chose wordC instead of optimal wordB');
      expect(report.suggestedPath).toEqual(['wordC', 'wordD']); // Path from last player position (wordC) to targetWord (wordD)
    });

    it('should handle zero optimal semantic distance for semanticPathEfficiency (e.g. start=end)', () => {
        const playerPath = ['wordA']; // Game ended at start or one move to itself
        const optimalPathGlobal = ['wordA'];
        const report = generateGameReport(mockGraphData, playerPath, optimalPathGlobal, [], 'wordA', findShortestPath, findShortestPath, [], []);
        expect(report.semanticPathEfficiency).toBe(100); // optimal 0 / player 0 (or some small value) should be 100%
        expect(report.status).toBe('won');
    });

    it('should handle zero player path distance for semanticPathEfficiency if optimal is non-zero', () => {
        const playerPath = ['wordA']; 
        const optimalPathGlobal = ['wordA', 'wordB']; // Optimal distance 0.2
        const report = generateGameReport(mockGraphData, playerPath, optimalPathGlobal, [], 'wordB', findShortestPath, findShortestPath, [], []);
        // Here, efficiencyDenominator for playerPath ['wordA'] to 'wordB' becomes dist of suggested ['wordA','wordB'] = 0.2
        // semanticPathEfficiency = (0.2 / 0.2) * 100 = 100. This seems correct if player made 0 moves but was at start of optimal.
        // If player given_up at start, suggestedPathFromFinalPosition is A->B (cost 0.2)
        // hypotheticalFullPath = [A,B]. efficiencyDenominator = 0.2
        // OptimalDistance = 0.2. (0.2/0.2)*100 = 100
        expect(report.semanticPathEfficiency).toBeCloseTo(100);
        expect(report.status).toBe('given_up');
    });
