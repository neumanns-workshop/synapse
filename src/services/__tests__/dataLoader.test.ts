...
describe("dataLoader", () => {
  ...
  describe('Data Consistency', () => {
    it('should have consistent words between graph and frequencies', async () => {
      ...
      const intersection = new Set([...graphWords].filter(x => frequencyWords.has(x)));
  ...
  describe('Data Validation', () => {
    ...
    it('should handle malformed data gracefully', async () => {
      ...
      jest.doMock('../../data/graph.json', () => ({
        ...
        invalidWord: { /* missing edges and tsne */ },
        anotherInvalid: { edges: 'not an object', tsne: 'not an array' },
      }));

      const { loadGraphData: malformedLoadGraphData } = require('../dataLoader');
