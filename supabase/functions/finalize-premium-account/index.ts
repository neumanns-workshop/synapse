...
serve(async (req) => {
  ...
  try {
    ...
    if (callingUserError || !callingUser) {
      ...
      return new Response(
        JSON.stringify({ error: "Authentication failed for calling user." }),
    ...
    if (!temporaryUserId || !email || !password) {
      return new Response(
        JSON.stringify({
          error:
            "Missing required parameters: temporaryUserId, email, password",
        }),
    ...
    if (profileError) {
      console.error(
        `Error fetching profile for temporaryUserId ${temporaryUserId}:`,
        profileError,
      );
      return new Response(
        JSON.stringify({
          error: "Failed to fetch user profile for verification.",
        }),
    ...
    if (!profile || !profile.is_premium) {
      console.warn(
        `Attempt to finalize account for non-premium user or user not found: ${temporaryUserId}`,
      );
      return new Response(
        JSON.stringify({
          error: "User is not marked as premium or does not exist.",
        }),
    ...
    if (updateAuthUserError) {
      console.error(
        `Error updating auth user ${temporaryUserId}:`,
        updateAuthUserError,
      );
      return new Response(
        JSON.stringify({
    ...
    if (profile.email !== email) {
      ...
      if (updateProfileEmailError) {
        ...
        console.warn(
          `User auth updated, but failed to update email in user_profiles for ${temporaryUserId}:`,
          updateProfileEmailError,
        );
      } else {
        console.log(
          `Successfully updated email in user_profiles for ${temporaryUserId} to ${email}`,
        );
      }
    }

    console.log(
      `Successfully finalized premium account for user ID: ${temporaryUserId}, email: ${email}`,
    );
    return new Response(
      JSON.stringify({
    ...
    if (
      ...
      error.message.includes("Failed to update user")
    ) {
      status = 500;
    }
    return new Response(
