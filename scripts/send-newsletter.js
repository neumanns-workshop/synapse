...
if (!supabaseUrl || !supabaseServiceKey) {
  ...
  console.log('Make sure you have EXPO_PUBLIC_SUPABASE_URL and EXPO_PUBLIC_SUPABASE_SERVICE_KEY set');
  process.exit(1);
}
...
if (!articleId) {
  ...
  console.log('Usage: node scripts/send-newsletter.js <article-id> [--dry-run]');
  console.log('Available articles:');
  
  // List available articles
  const articlesPath = path.join(__dirname, '../src/data/news/posts');
  const files = fs.readdirSync(articlesPath).filter(f => f.endsWith('.ts') && f !== 'index.ts');
  files.forEach(file => {
...
async function sendNewsletter() {
  try {
    // Load the article
    const articlePath = path.join(__dirname, `../src/data/news/posts/${articleId}.ts`);
    ...
    if (!titleMatch || !contentMatch || !dateMatch) {
      console.error('❌ Could not parse article data. Check the article format.');
      process.exit(1);
    }
    ...
    console.log(`📧 ${isDryRun ? 'DRY RUN - ' : ''}Sending newsletter: "${articleData.title}"`);
