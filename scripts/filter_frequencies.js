...
const DEFINITIONS_DATA_PATH = path.join(__dirname, '../src/data/definitions.json');
const WORD_FREQUENCIES_PATH = path.join(__dirname, '../src/data/word_frequencies.json');
const FILTERED_FREQUENCIES_OUTPUT_PATH = path.join(__dirname, '../src/data/filtered_word_frequencies.json');
...
try {
  ...
  if (!graph) {
    throw new Error('Could not find "nodes" key in graph.json. Please ensure the structure is { "nodes": { ...graph_data... } }');
  }
  ...
  for (const word of gameWords) {
    if (allFrequencies.hasOwnProperty(word)) {
      filteredFrequencies[word] = allFrequencies[word];
      count++;
    }
  }
  console.log(`Filtered frequencies: ${count} words retained out of ${Object.keys(allFrequencies).length} total original frequencies.`);

  // 4. Save Filtered Data
  fs.writeFileSync(FILTERED_FREQUENCIES_OUTPUT_PATH, JSON.stringify(filteredFrequencies, null, 2)); // null, 2 for pretty printing
  console.log(`Filtered word frequencies saved to: ${FILTERED_FREQUENCIES_OUTPUT_PATH}`);
