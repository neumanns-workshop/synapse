...
function generateThemedChallenges(
  ...
  while (challenges.length < targetCount && attempts < maxAttempts) {
    ...
    const randomCluster =
      clusters[clusterNames[Math.floor(Math.random() * clusterNames.length)]];

    // Pick random start and target words
    const startWord =
      randomCluster.starters[
        Math.floor(Math.random() * randomCluster.starters.length)
      ];
    const targetWord =
      randomCluster.targets[
        Math.floor(Math.random() * randomCluster.targets.length)
      ];
    ...
    if (
      challenges.some(
        (c) => c.startWord === startWord && c.targetWord === targetWord,
      )
    ) {
      continue;
    }
    ...
    const { url, hash } = generateChallengeUrl(
      startWord,
      targetWord,
      themeName,
    );
    ...
    const description =
      randomCluster.descriptions[
        Math.floor(Math.random() * randomCluster.descriptions.length)
      ];
    ...
    console.log(
      `✓ Generated: ${startWord} → ${targetWord} (${optimalPathLength} moves)`,
    );
  }
  ...
  return {
    ...
    description:
      "Themed challenges for guerrilla marketing QR codes - OKC to Santa Fe summer road trip",
    ...
    themes: [
      {
        ...
        name: "Summer Vibes",
        description:
          "Chase the sun on endless summer adventures from OKC to Santa Fe",
        challenges,
      },
    ],
  };
...
async function main() {
  try {
    ...
    console.log(
      `🎯 Success rate: ${((output.totalChallenges / 500) * 100).toFixed(1)}%`,
    );
  } catch (error) {
